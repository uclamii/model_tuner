...
def generate_classification_data(n_samples: int = 100, n_features: int = 20, n_classes: int = 2, random_state: int = 42) -> tuple[pd.DataFrame, pd.Series]:
...
def test_check_input_type() -> None:
    ...
    assert isinstance(check_input_type(np_array), pd.DataFrame), "Failed to convert np.array to DataFrame"
    assert isinstance(check_input_type(pd_series), pd.Series), "Failed to handle pd.Series correctly"
    assert isinstance(check_input_type(pd_dataframe), pd.DataFrame), "Failed to handle pd.DataFrame correctly"
...
def test_sampling_method() -> None:
    ...
    assert len(balanced.value_counts()) == 2, "Number of classes are not 2, sampling failed"
    ...
    assert custom.value_counts()[0] == 3, "Custom proportion sampling for class 0 failed"
    assert custom.value_counts()[1] == 3, "Custom proportion sampling for class 1 failed"
